function Modele095({
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 top-40 h-2 w-32 rotate-[-25deg] bg-white/10" />
      <div className="absolute right-0 top-52 h-2 w-40 rotate-[25deg] bg-white/10" />
      ...
      <div className="absolute left-5 right-5 top-5 flex justify-center">
  <div className="flex items-center gap-3 rounded-2xl border border-white/20 bg-black/30 px-4 py-2 backdrop-blur">
    {logoFinal ? (
      <img src={logoFinal} alt={Logo } className="h-10 w-10 rounded-lg object-contain" />
    ) : (
      <span className="text-xl" style={{color: palette.accent}}>✦</span>
    )}

    <span className="max-w-[190px] truncate text-[9px] font-black uppercase tracking-[0.2em] text-white">
      {nom}
    </span>
  </div>
</div>
